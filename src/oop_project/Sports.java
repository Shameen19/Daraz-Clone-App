...
public class Sports implements Serializable
{
    ...
    public Sports(String code,String brand, String quantity, String name,String price)
    {
        this.code=code;
        this.brand=brand;
    ...
    public  static void writeProduct(Sports sport)
    {
        ArrayList<Sports> list = readAllDataFromFile();
        list.add(sport);
        try
        {
            ObjectOutputStream output = new ObjectOutputStream( new FileOutputStream("Sport Record"));
    ...
    public  static  ArrayList<Sports> readAllDataFromFile(   )
    {
        ArrayList <Sports> list = new ArrayList<Sports>();
        
        try
        {
            ObjectInputStream input = new ObjectInputStream (new FileInputStream("Sport Record"));
            list = (ArrayList<Sports>) input.readObject();
        }
    ...
    {
        ArrayList <Sports> list = readAllDataFromFile();
        System.out.println("SPORT CODE\t\tSPORT BRAND\t\tSPORT NAME\t\tSPORT QUZNTITY\t\tSPORT PRICE");
        for(int i=0;i<list.size();i++)
        {
          System.out.println(list.get(i).getCode()+"\t\t\t"+list.get(i).getBrand()+"\t\t\t"+list.get(i).getName()+"\t\t\t"+list.get(i).getQuantity()+"\t\t\t"+list.get(i).getPrice());  
        }
